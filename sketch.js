...
// Select how many loops are used, the size of the squares and the number of steps
var loop_samples = 21;
var w = 35;
var steps = 8;

// In cells we store the selected squares, initialize empty
var cells = [];
...
function playBeat(time) {
  // Send the kit with sounds to master
  kit.toMaster();
...
// detect what sqaure was selected
  ...
  for (var step = 0; step < steps; step++) {
    for (var track = 0; track < loop_samples; track++) {
      if (
        ...
        if (cells[track][step] == 1) cells[track][step] = 0;
        else cells[track][step] = 1;
      }
